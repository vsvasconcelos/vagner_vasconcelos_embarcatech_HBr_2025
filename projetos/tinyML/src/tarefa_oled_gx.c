...
void tarefa_oled_gx(void *params) {
    sensor_dataf_t media;

    printf("[tarefa_oled_gx] Iniciada\n");
    ...
    while (true) {
        if (buffer_sensor_calcular_media(&media)) {
            if (xSemaphoreTake(mutex_oled, portMAX_DELAY)) {
                ...
                oled_centralizar_texto(&oled, "GIROSCÓPIO", 1);  // Linha 0
                oled_centralizar_texto(&oled, "Gx", 2);            // Linha 1
                oled_centralizar_texto(&oled, "(º/s)", 3);           // Linha 2

                // Prepara string com sinal e duas casas decimais
                snprintf(texto_valor, sizeof(texto_valor), "%+7.2f", media.gx);
