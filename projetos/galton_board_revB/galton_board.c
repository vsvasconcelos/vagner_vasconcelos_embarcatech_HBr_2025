...
int main() {
    ...
    void imprime_lcd(int texto, int coluna, int linha, int delay){
        char aux_Str[1];
        ...
        if (botaoA_flag == 1){
            ...
            for(int w = 1; w <= Bolas ; w++){
                ...
                for(int x = 1; x <= colunas; x++){
                    ...
                    } else if ((posicao_joystick >= 2200)|| (posicao_joystick <= 2000)){
                        col++;
                    }
                    row++;
                    if(row == (colunas-1)) {
            ...
            imprime_lcd(counter[0],21,57,40);
            imprime_lcd(counter[1],41,57,40);
            imprime_lcd(counter[2],61,57,40);
            imprime_lcd(counter[3],81,57,40);
            imprime_lcd(counter[4],101,57,40);
            ssd1306_send_data(&ssd);
            botaoA_flag=0;
            sleep_ms(10000);
        }    
